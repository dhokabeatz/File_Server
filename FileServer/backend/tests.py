...
class AuthTests(TestCase):
    ...
    def test_access_user_dashboard_unauthenticated(self):
        ...
        response = self.client.get(reverse('userDashboard'))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, f"{reverse('login')}?next={reverse('userDashboard')}")
...
class CustomUserCreationFormTest(TestCase):
    ...
    def test_duplicate_email(self):
        CustomUser.objects.create_user(username='existinguser', email='test@example.com', password='A!s2d#f4G%h')
        form = CustomUserCreationForm(data={
            'username': 'newuser',
            'email': 'test@example.com',
            'password1': 'A!s2d#f4G%h',
...
class DocumentModelTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        Document.objects.create(title='Test Document', description='This is a test document', file='testfile.txt', uploaded_by=user)
